...
### Missing Values
Missing data is way more common than I expected. I practiced three approaches:
...
I didn't realize how messy real data can be — same customer appearing twice with slightly different names, city names written in both uppercase and lowercase, etc. `df.duplicated()` finds exact duplicates, but inconsistent strings need manual cleaning with `.str.strip().str.lower()`.

### Outlier Detection
Used the IQR method: anything below `Q1 - 1.5*IQR` or above `Q3 + 1.5*IQR` is considered an outlier. Visualizing with boxplots before and after cleaning was really helpful to see the effect.
...
### Data Visualization
Practiced the main chart types:
...
Built a correlation matrix and visualized it as a heatmap with `seaborn`. High correlation between two features means they carry similar information — useful to keep in mind for feature selection in ML.

### Data Storytelling
The last section was about turning analysis into business insights — not just "correlation is 0.85" but "customers who use credit cards tend to spend 30% more per order." This was my favorite part.
...
- Visualization is not just for presentation; it helps *you* spot problems in the data.
